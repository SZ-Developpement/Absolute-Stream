...
import * as SliderPrimitive from "@radix-ui/react-slider"; // Correction de l'import pour que ça tourne
...
function Slider({
  ...
  min = 0,
  max = 100,
  color = "#E50914", // Couleur par défaut (ex: Rouge Netflix)
  ...props
    ...
    >
      ...
      {Array.from({ length: _values.length }, (_, index) => (
        <SliderPrimitive.Thumb
          ...
          key={index}
          className="block size-4 shrink-0 rounded-full border border-primary bg-white shadow-sm ring-white/20 transition-[color,box-shadow] hover:ring-4 focus-visible:ring-4 focus-visible:outline-hidden disabled:pointer-events-none disabled:opacity-50"
        />
      ))}
    </SliderPrimitive.Root>
  );
